...
Two facts about English kinship terms have been documented extensively. The address/reference literature and the determiner-syntax literature have largely developed separately. While work on conversational reference already approaches the interface \citep{Downing1996}, an explicit mechanism linking the parent-name taboo to determinerless argument structure hasn't been developed.
...
The mechanism predicts that address norms should precede bare-argument licensing diachronically, but this prediction is unlikely to be resolvable from the English historical record. Both the naming taboo and bare kinship-term use probably predate the available corpora, and the process is primarily a spoken-language phenomenon poorly reflected in written texts. The causal claim rests on synchronic distributional evidence~-- the gradient's alignment with taboo strength, the correlation between vocative concentration and bare-argument rates~-- and on the plausibility of the proposed pathway rather than on demonstrated historical sequence.
