...
\midrule
Proper noun & Noun with proper-name function & \mention{John}, \mention{Mom} (bare) \\
Proper name & NP with unique definite reference & \mention{the United Kingdom} \\
...
\emph{The Cambridge Grammar of the English Language} \citep{Huddleston2002} distinguishes carefully between lexical category and semantic function. A \term{proper noun} is a lexical category~-- a word class with distinctive morphosyntactic properties. A \term{proper name} is a semantic function~-- an NP that picks out a unique, identifiable referent. The two can come apart.
...
CGEL further distinguishes \term{strong proper names}~-- those that appear without an article (\mention{Paris}, \mention{John})~-- from \term{weak proper names}~-- those that require \mention{the} (\mention{the Thames}, \mention{the Hague}). Strong proper names exhibit non-deital morphosyntax: they're semantically definite (unique, identifiable) but appear bare, without the determiner that typically marks definiteness.
