...
Third, \emph{high frequency}: this isn't occasional use. Children say \mention{Mom} and \mention{Dad} with extreme frequency over childhood~-- even 30--50 uses per day yields 200,000--300,000 tokens over 15--20 years. The frequency is amplified by parent-directed speech: caregivers routinely refer to \emph{themselves} as \mention{Mommy} and \mention{Daddy} (\mention{Mommy's going to the store}), providing massive input frequency in name position. Each use is in a slot prototypically occupied by proper names.
...
\subsection{Corpus evidence: CHILDES}
...
Standard Malagasy has obligatory determiners \citep{Keenan2008}. Common nouns require the definite article \mention{ny} (\mention{ny hazo} = `the tree'); bare common nouns are interpreted as indefinite. Proper nouns require a distinct set of personal determiners: informal \mention{i}, polite \mention{ra}, or honorific \mention{andria}.

Crucially for the present argument, Malagasy also has a dedicated vocative determiner \mention{ry} that applies to both proper and common nouns \citep{Potsdam2010}. Compare \mention{i Soa} (`Soa', non-vocative) with \mention{ry Soa} (`Soa!', vocative), and \mention{ny mpianatra} (`the students') with \mention{ry mpianatra} (`students!'). This contrasts sharply with English, where vocative position is bare (\mention{Mom, come here!}), not marked by a special determiner. The distinction matters: in English, vocative use and bare-NP syntax converge; in Malagasy, vocative position has its own morphological realization.

\subsection{The naming taboo}

Among the Zafimaniry of Madagascar, naming taboos are far more pervasive than in English \citep{Bloch2006}. Personal names progressively fall out of use as speakers acquire social status:

\begin{itemize}
    \item \emph{Young parents}: Personal names become awkward. Teknonyms (\mention{renin'Solo} = `mother of Solo') replace them in address and reference.
    \item \emph{Established elders} (\term{raiamandreny}): Personal names are actively avoided. Using one would deny the person's legitimate place in the moral order.
    \item \emph{Full elder-hood}: Even teknonyms give way to village-based reference. A person is addressed simply by the name of their village~-- complete depersonalization.
\end{itemize}
...
The existence of a dedicated vocative determiner refines the prediction. In English, vocative position is bare, so vocative use and proper-name-like syntax converge in a single morphological form. In Malagasy, they can diverge: vocative position may take \mention{ry} or appear bare \citep{Paul2009}, while argument position requires either \mention{ny} (common noun) or \mention{i}/\mention{ra} (proper noun).
