...
Three methodological caveats are in order. First, the title+name exclusion was motivated by linguistic analysis (the proper noun, not the kinship term, is the syntactic head), not by its effect on the correlation, but it was not pre-registered, and the magnitude of improvement should be interpreted with that in mind: other defensible operationalizations might yield different values of $\rho$. Second, with $n = 30$ terms the correlation is suggestive but not dispositive; the bootstrap interval honestly reflects the uncertainty that a small sample entails. Third, the correlation is consistent with the proposed mechanism but does not establish its causal direction. The causal argument rests on the theory and on the gradient's alignment with taboo-strength predictions, not on the $\rho$ value itself.
