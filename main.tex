...
\subsection{Typological context}

The English naming taboo is not an isolated curiosity. Cross-linguistically, social taboos frequently constrain reference. Austronesian death taboos prohibit names of the deceased; Nguni \term{hlonipha} restricts vocabulary for married women; Chinese \term{bìhuì} traditionally blocked names of emperors and ancestors \citep{Braun1988,Adamek2012}.

However, these taboos often drive \emph{lexical replacement}: words are avoided and die out. The English parent-child taboo drives \emph{frequency}: the kinship term is forced into the vacuous slot, leading not to loss but to grammaticalization.
...
\subsection{The typological trade-off}

Testing the mechanism strictly requires a typological trade-off. The naming taboo is strongest in East Asian languages, which often lack the obligatory determiner systems (deital clusters) needed to test the bare-singular prediction. Conversely, the determiner system is strictly enforced in Romance languages, where the taboo is moderate. We therefore triangulate: East Asian languages illustrate the \emph{input intensity} (how powerful the taboo can be), while Romance languages provide the \emph{syntactic precision} (how the mechanism overrides strict constraints).

\subsection{Input intensity: Chinese and Vietnamese}

In Mandarin Chinese, traditional \term{bìhuì} (\enquote{name avoidance}) prohibited speaking or writing the names of emperors and ancestors \citep{Adamek2012}. Children historically couldn't use parents' names at all. The kinship system is correspondingly elaborate, with distinct terms for maternal/paternal and older/younger distinctions. Vietnamese exhibits similar complexity, with kinship terms functioning as the primary pronoun system \citep{Luong1990}.

These systems demonstrate the sheer force of the avoidance variable. While we cannot test \enquote{determiner drop} in languages without articles, the pervasive displacement of names by kinship terms in these languages confirms that social prohibitions can fundamentally restructure the nominal domain.

\subsection{Alternative strategies: Arabic technonymy}

The naming taboo doesn't always produce bare kinship terms. In Arabic, the taboo on using a senior's name drives the formation of \term{technonyms} (the \mention{kunya} construction): parents are addressed as \mention{Abu X} (\enquote{Father of X}) or \mention{Umm X} (\enquote{Mother of X}), where X is the eldest son \citep{Braun1988}.

This strategy solves the same social problem~-- avoiding the first name~-- but via a different grammatical route. Instead of stripping a kinship term to make it name-like (the English/French strategy), the language constructs a new name-like title. This highlights that while the \emph{social pressure} is universal, the \emph{grammatical solution} is language-specific.

\subsection{Syntactic resolution: Romance and Germanic}

To test the specific hypothesis of proper-name grammaticalization (determiner drop), we need languages with strict article systems. French and German provide the crucial data.

French bare singulars are categorically ungrammatical (\ungram{\mention{Table est là}} is impossible). Yet children address parents as \mention{Maman} and \mention{Papa}, and these forms function as bare proper names in argument position:
\ea
    \ea[]{{\mention{Maman a dit non.}} \hfill (grammatical proper-name usage)}
    \ex[*]{{\mention{Mère a dit non.}} \hfill (ungrammatical common noun)}
    \z
\z

German shows a parallel pattern. Standard German requires articles for count nouns (\mention{Der Tisch}, not \ungram{\mention{Tisch}}). However, \mention{Mutti} and \mention{Vati} behave exactly like proper names (\mention{Hans}, \mention{Maria}): they appear bare in subject position (\mention{Mutti hat angerufen}).

This European cluster offers the necessary syntactic traction. In both language families, the taboo forces a specific lexical item into the name slot, and despite the language's general prohibition on bare singulars, that item acquires the syntactic privileges of a proper name.
...
The honorific prediction implies that title-like kinship terms should also acquire proper-name syntax. In French, combined forms like \mention{Tante Sarah} or \mention{Oncle Jean} function as proper names and appear without determiners, contrasting with the obligatory determiner in \mention{La tante Sarah} (referring to her oppositionally).
