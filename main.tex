...
\section{Strategies of repair: A typological view}\label{sec:crossling}

The English naming taboo is not unique, but its solution is specific. Cross-linguistically, the problem of avoiding a name is universal, but languages repair this problem through three distinct strategies varying in grammatical intensity. English represents the subtle end of a spectrum that extends to radical lexical replacement.

\subsection{Strategy A: Lexical replacement (The nuclear option)}

In Australian languages like Dyirbal, the taboo against naming distinct affines (e.g., mothers-in-law) is absolute. It triggers \term{Dyalnguy} ("mother-in-law language"), a completely separate vocabulary used in the presence of taboo relatives \citep{Dixon1972}. Every open-class word in the everyday language (\term{Guwal}) has a distinct Dyalnguy equivalent. The taboo is so powerful it forces the speaker to switch lexicons entirely.

This strategy operates at the level of the \emph{lexical inventory}. The name is not just avoided; the language itself changes to index the avoidance.

\subsection{Strategy B: Constructional bypass (Technonymy)}

A moderate strategy involves building a new referring expression. In Arabic, the taboo on using a senior's name drives \term{technonymy} (\mention{kunya}): parents are addressed as \mention{Abu X} or \mention{Umm X} ("Father/Mother of X") \citep{Braun1988}.

Here, the lexicon remains stable, but the grammar provides a \emph{constructional bypass}. The speaker avoids the names \mention{Ahmed} or \mention{Layla} by constructing a title-like descriptions. It is a morphological solution to a pragmatic problem.

\subsection{Strategy C: Syntactic recategorization (The European solution)}

English, French, and German employ the subtlest strategy: \term{syntactic recategorization}. The taboo forces a relational noun (\mention{mother}, \mention{Maman}, \mention{Mutti}) into the name slot. The word doesn't change (unlike Dyirbal), and no new title is constructed (unlike Arabic). Instead, the \emph{syntax} changes: the noun sheds its determiner and behaves as a proper name.

\ea
    \ea[] English: {\mention{Mom called.}} \hfill (bare singular)
    \ex[] French: {\mention{Maman a dit non.}} \hfill (vs.\ ungrammatical \mention{*Mère})
    \ex[] German: {\mention{Mutti hat angerufen.}} \hfill (vs.\ ungrammatical \mention{*Tisch})
    \z
\z

This European cluster confirms that where the syntactic machinery (articles) exists, the taboo creates a distinct grammatical category. The English "bare mom" is not a casual omission; it is the grammatical signature of a specific repair strategy~-- less radical than Dyirbal, less constructional than Arabic, but structurally distinct.
