...
The argument is simple: the social prohibition on first-name use forces high-frequency use of kinship terms in vocative and address function. Vocative function is determiner-free\footnote{Vocative NPs permit 1st- and 2nd-person pronoun genitives (\mention{My friend!}, \mention{Your majesty!}) and the 2nd-person determinative (\mention{You idiot!}), but exclude all other determinatives and all other genitives, including 3rd-person pronouns, common nouns, and proper nouns as determiners. For kinship terms, bare use is standard.}~-- you say \mention{Mom!}, \mention{Waiter!}, \mention{Taxi!}, not \ungram{\mention{The mom!}, \mention{That waiter!}, \mention{Some taxi!}}~-- and this syntactic property provides the bridge to proper-noun status. That frequency, sustained across childhood and into adulthood, produces exactly the conditions under which grammaticalization toward proper-noun-like syntax occurs. The taboo is proposed as a primary driver, operating through distributional exposure in vocative contexts; other factors may contribute, but the taboo provides the dominant frequency asymmetry.
...
This mechanism predicts a sharp divergence between parent terms and extended kin. Corpus data confirm this prediction: parent terms show 16--43\% vocative use in CHILDES (reflecting variation across individual lexical items; see Table~\ref{tab:childes}), compared to only 4--7\% for extended kin terms. This statistical asymmetry provides the usage-based engine for the syntactic shift.
...
\subsection{The \mentionhead{CGEL} framework}
...
Term & Gloss & Example \\
\midrule
Proper noun & Lexeme: Noun serving as proper name & \mention{John}, \mention{Clinton} \\
...
Proper names occupy a distinctive position in this framework: they're \emph{semantically definite} (they refer to unique, identifiable individuals) and show the distributional restrictions of deital NPs (resisting existential pivots, participating in partitives), but they lack the primary morphological exponent~-- no article appears in \ungram{\mention{the John}}. This makes them peripheral members of the deitality cluster, not outsiders to it. This paper brackets the philosophical semantics of direct reference \citep{Kripke1980}, focusing instead on the morphosyntactic properties. English kinship terms, when functioning as proper names, pattern exactly this way: they acquire the distributional properties of proper nouns~-- grammaticalized fossils of the taboo that shaped their use.

This explains why \mention{Mom called} behaves more like \mention{Paris welcomed visitors} than like \mention{My mother called}. Both \mention{Mom} and \mention{Paris} are strong proper names: definite in interpretation, bare in form but showing the distributional hallmarks of deitality. The kinship term has been pulled toward proper-noun status by the social pressure of address.
...
The result is massive frequency differential. A child says \mention{Mom} or \mention{Dad} thousands of times per year, every year, from first words through adulthood. Each use occurs in a slot prototypically occupied by proper names~-- vocative function, subject of address verbs, the referential slot in family conversations. The taboo doesn't just prohibit one form; it channels usage toward another, creating the conditions for grammaticalization.

\section{Kinship terms with proper-noun status}\label{sec:syntax}
...
\textcite{Hill2022} adds a structural account: vocative phrases have dedicated syntactic positions, and forms that occupy those positions share grammatical properties. Hill's analysis is particularly relevant here: she argues that vocative phrases morphosyntactically encode social superiority, and that kinship rank specifically plays a role in establishing speaker dominance. Kinship terms in vocative function thus inherit both the syntactic properties of vocative phrases and the social-indexical function of kinship rank. The vocative slot is a grammaticalization highway~-- a context where high-frequency forms acquire special privileges.
...
\ea\label{ex:pathway}
    ...
    \ex Stage 4 (Target construction): strong proper name (proper-noun status; bare but distributionally deital): \mention{Mom} called.
    \z
\z
...
The kinship story isn't unique to kinship. It's an instance of a general mechanism~-- frequency in name position produces proper-noun syntax~-- applied to a case where social prohibition creates the requisite frequency.
...
This paper has connected the sociolinguistic naming taboo to its specific syntactic consequence: the proper-noun syntax of kinship terms (\mention{Mom called} is grammatical where \ungram{\mention{Neighbour called}} isn't in ordinary conversation). The connection is causal. The taboo creates the frequency; the frequency creates the syntax.
...
Third, the paper offers falsifiable predictions supported by initial empirical validation. Corpus data from CHILDES confirm that parent terms show disproportionate vocative concentration (16--43\%) compared to extended kin terms (4--7\%), and vocative concentration predicts bare-argument realisation across terms. Across kinship terms, gradience in proper-noun syntax tracks address frequency. Across languages, the strength of the naming taboo should predict the robustness of proper-noun properties for kinship terms. Within communities, weakening of the taboo should correlate with weakening of proper-noun syntax.
