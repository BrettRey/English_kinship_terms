...
For those working within a DP framework, \textcite{Longobardi1994} analyses proper names in Romance as involving N-to-D movement, with bare argument use licensed by a null D carrying [+proper] features. On such an account, the question becomes what determines which nouns acquire those features~-- a question the formal apparatus doesn't answer but the frequency mechanism does. The two approaches address different questions (structural licensing vs.\ distributional explanation), though they rest on different assumptions: Longobardi's discrete binary features sit uneasily with the gradience documented here, and the DP hypothesis itself is not adopted in the \emph{CGEL} framework this paper follows.
