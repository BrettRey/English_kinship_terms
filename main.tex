...
\section{Cross-linguistic evidence: Malagasy}\label{sec:crossling}
...
Standard Malagasy has obligatory determiners \citep{Keenan2008}. Common nouns require the definite article \mention{ny} (\mention{ny hazo} = `the tree'); bare common nouns are interpreted as indefinite. Proper nouns require a distinct set of personal determiners: informal \mention{i}, polite \mention{ra}, or honorific \mention{andria}. The system is comparable in strictness to French.
...
If the mechanism proposed for English is correct, Malagasy teknonyms and kinship-based address forms should show correspondingly robust proper-name syntax. Specifically:
\begin{itemize}
    \item Teknonyms should pattern with proper nouns for determiner selection (requiring \mention{i}/\mention{ra} rather than \mention{ny}).
    \item High-frequency teknonyms should resist nominal modification.
    \item The gradient from teknonymy to village-reference should parallel a gradient in grammaticalization.
\end{itemize}

Detailed syntactic work on Zafimaniry teknonyms remains to be done. The prediction is precise: the intensity of the taboo should produce correspondingly robust name-like properties, tested via diagnostics appropriate to the Malagasy determiner system. This is exactly the cross-linguistic test the mechanism demands.
