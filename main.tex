...
\ea\label{ex:pathway}
    \ea Stage 1 (Source construction): relational NP headed by a common noun with definite determiner: \mention{my mom}
    ...
    \ex Stage 4 (Target construction): strong proper name (proper-name status; non-definite, bare NP in argument position): \mention{Mom} called.
    \z
\z
...
Vietnamese presents a striking case. Kinship terms function as the primary pronoun system, with speakers selecting terms based on relative age and social relationship. When addressing a woman older than one's mother, one uses \mention{bác} (\enquote{aunt, older than parent}); when addressing a younger woman in a teacher role, one uses \mention{cô} (\enquote{aunt, younger than parent}). These terms extend beyond biological kin to strangers, functioning as honorifics. The prediction is that such terms should show strong proper-name properties~-- and indeed, they appear without determiners in positions where relational nouns would require them.
