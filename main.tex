...
Here is where the English taboo diverges from other naming taboos typologically. Most naming taboos drive \emph{lexical replacement}: the prohibited word is avoided and eventually dies out, replaced by circumlocutions or neologisms. Zulu hlonipha, discussed above, is the paradigm case: avoidance of in-law name syllables has reshaped the lexicon. The English parent-child taboo works differently. It doesn't prohibit the kinship term~-- it prohibits the \emph{alternative}. The first name is blocked; the kinship term is forced into the vacancy. The result is not lexical loss but frequency concentration~-- and frequency concentration, on standard usage-based accounts, is the engine of grammaticalization.

What matters for the grammatical story is the consequence of the taboo for usage frequency. Because children can't use parents' first names, they have to use kinship terms~-- a constrained selection with the constraint enforced by social pressure, not a free choice among alternatives. This creates a distributional monopoly in name position that ordinary frequency effects don't produce.
...
The exceptions reveal the pattern. Proper nouns resist determiners: \mention{Paris is beautiful}, not \ungram{\mention{The Paris is beautiful}}. Bare singulars in argument position are the province of proper nouns: forms that have escaped the determiner requirement because they function as strong proper names. Institutional bare singulars (\mention{go to school}, \mention{in hospital}) arise through a different pathway~-- semantic bleaching rather than proper-noun grammaticalization~-- and remain prepositionally bound.
...
The parallel also extends to distributional diagnostics. Section~\ref{sec:intro} introduced deitality as a morphosyntactic property cluster distinct from semantic definiteness. Two classic diagnostics for deitality are resistance to existential pivots and participation in partitives. Bare \mention{Mom} passes both: \ungram{\mention{There's Mom waiting outside}} is odd (existential-pivot resistance), while \mention{most of Mom's friends} is fine (partitive participation). Crucially, adding a possessive rescues the existential: \mention{There's my mom waiting outside} is fine, showing that the constraint is on the bare proper-name form, not on the pragmatics of identifiable referents. These judgments match prototypical proper names like \mention{John}: \ungram{\mention{There's John waiting outside}} is similarly degraded, while \mention{most of John's friends} is unexceptionable.

A third diagnostic involves modification. Common-noun NPs accept attributive modifiers freely: \mention{my tired mom}, \mention{my exhausted mother}. Bare kinship proper names resist them: \ungram{\mention{Tired Mom called}} is degraded, parallel to \ungram{\mention{Tired John called}}. The constraint follows from proper-noun status: proper names resist modification because they already denote a unique individual; adding an attributive implies a contrast set that proper names don't license. Kinship proper names inherit this restriction.
...
\mention{Auntie} and \mention{Uncle} occupy an intermediate position. Unlike most English honorifics, which combine with surnames (\mention{Dr.\ Smith}, \mention{Mr.\ Jones}), these combine with first names (\mention{Auntie Sarah}, \mention{Uncle John}). This quasi-titular use gives them name-position exposure that siblings lack: bare usage (\mention{Auntie}) can be analyzed as an elision of the full title-plus-name construction (\mention{Auntie \textup{[}Sarah\textup{]}}), providing a bridge for proper-noun syntax. The formal variant \mention{Aunt} lacks this quasi-titular use and accordingly shows no proper-noun properties: the form that gets name-position exposure (\mention{Auntie}) acquires proper-noun syntax; the form that doesn't (\mention{Aunt}) doesn't. This is a minimal pair the semantic-uniqueness account cannot explain: \mention{Auntie} and \mention{Aunt} have the same denotation, but only the form with name-position exposure shows proper-noun distribution (see \S\ref{sec:objections} for the semantics objection). \mention{Brother} and \mention{Sister} are unlicensed in ordinary family discourse but can function as titles in religious contexts (\mention{Brother John}, \mention{Sister Mary}). In Quaker communities, for instance, bare \mention{Sister} appears in subject position: \enquote{Sister said we could make no such engagements} \citep[qtd.\ in][]{Hodgkin1927}. The mechanism generalizes beyond nuclear-family kinship.
...
Bare-argument use tracks vocative concentration. Bare arguments were defined as argument uses without overt determiners or genitive markers. Apostrophe-\mention{s} was treated as genitive only when immediately followed by a noun without copular/auxiliary continuation (e.g., \mention{mom's car}); all other contexts, including predicate adjectives (\mention{Mom's mad}), PPs (\mention{Mom's in the kitchen}), and copular NPs (\mention{Mom's the boss}), were treated as contractions and counted as bare arguments. Using terms with at least 50 argument tokens, vocative percentage correlates with bare-argument percentage (Pearson $r=0.72$; Spearman $\rho=0.73$; $n=30$). The correlation may be partly driven by the categorical parent/extended split; whether a continuous dose-response relationship holds within either category is a question for finer-grained data. Still, the correlation holds when collapsing morphological variants into family clusters (MOM, DAD, etc.) and when restricting to child-speaker tokens only. Figure~\ref{fig:vocative-bare} plots this relationship: terms with more vocative concentration are also the ones most likely to appear as bare arguments.
...
\caption{Bare-argument rate by kinship term (terms with $\geq 50$ argument tokens), ordered by rate. The gradient tracks taboo-driven vocative exposure: informal parent terms (highest, 60--90\%), grandparent terms (intermediate, 60--77\%), extended kin terms (lowest, 30--50\%). Title+name constructions (\mention{Auntie Sarah}, \mention{Uncle John}) are excluded from bare counts (since the name is the syntactic head) using \%mor-tier proper-noun tagging.}
\label{fig:gradient}
\end{figure}
...
To our knowledge, Malagasy is the \emph{only} typological profile that fully disentangles the confounds in the English data: overt proper-name determiners separate proper-noun status from mere definiteness, and overt vocative morphology separates vocative function from argument licensing. The prediction is sharp enough that even a small corpus study would be decisive. Detailed syntactic work on Zafimaniry teknonyms remains to be done.
...
Historical evidence is thin but suggestive. Hierarchical address norms~-- including constraints on parent-child address~-- have deep roots in English-speaking communities, while bare-singular kinship terms in argument position aren't clearly attested until later \citep[cf.][on the general persistence of hierarchical address]{BrownGilman1960}. More decisive evidence would require close study of Old and Middle English kinship-term distribution. For now, the diachronic question remains open, though the mechanism proposed here is consistent with the available trajectory evidence.
...
Second, the paper provides an explanatory mechanism, not just a description. Other treatments note that kinship terms can function as proper names; this paper explains \emph{why}~-- and where semantic-uniqueness accounts leave the gradient unexplained, including within minimal pairs like \mention{Auntie}/\mention{Aunt} that control for semantic content. The naming taboo is the engine; the frequency is the transmission; the syntax is the sediment.

Third, the paper offers falsifiable predictions, some already validated and others awaiting test. Corpus data from CHILDES confirm that parent terms show disproportionate vocative concentration (16--43\%) compared to extended kin terms (4--7\%), and that children drive the vocative skew (68\% of vocative uses come from child speakers, not caregivers). Vocative concentration correlates with bare-argument realisation across terms. Untested predictions follow: across languages, the strength of the naming taboo should predict the robustness of proper-noun properties (with Malagasy teknonyms providing the sharpest test); within families, weakening of the taboo should track weakening of proper-noun syntax for the same term; and the mechanism should extend to adult adoption of kinship terms in in-law relationships.
