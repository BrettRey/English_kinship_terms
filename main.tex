...
This isn't polysemy in the usual sense. It's positional: the same lexical item~-- a common noun~-- acquires different syntactic privileges depending on whether it's functioning semantically as a proper name or retaining its default relational semantics. The grammaticalization story explains \emph{why} kinship terms developed this split: the taboo forces them into proper-name function so frequently that they've acquired the syntactic distributional properties of proper nouns.
...
\emph{The Cambridge Grammar of the English Language} \citep{Huddleston2002} distinguishes carefully between lexical category and naming function. A \term{proper noun} is a lexical category~-- a word class specialized for naming. A \term{proper name} is a phrase-level expression~-- an NP conventionally adopted as the name of a unique entity. The two can come apart.

Consider \mention{the United Kingdom}. The noun \mention{kingdom} is a common noun, but the NP \mention{the United Kingdom} functions as a proper name: it refers to a unique political entity. The lexical category is common; the semantic function is naming.

CGEL further distinguishes \term{strong proper names}~-- those that appear without an article (\mention{Paris}, \mention{Mom})~-- from \term{weak proper names}~-- those that require \mention{the} (\mention{the Thames}, \mention{the Hague}). The terminology requires care: \mention{strong proper name} describes a syntactic configuration (a bare NP functioning as a name), not the lexical category of the word inside it. Strong proper names exhibit non-deital morphosyntax: they're semantically definite (unique, identifiable) but appear bare, without the determiner that typically marks definiteness.
...
\ea\label{ex:gradient}
    ...
    \ex[*]{Unlicensed: \mention{Cousin}, \mention{Nephew}, \mention{Sister-in-law}}
    \z
\z

The hierarchy correlates with address frequency. \mention{Mom} and \mention{Dad}~-- the kinship terms forced into highest-frequency use by the naming taboo~-- show the most robust proper-name syntax. Terms used less frequently in address exhibit weaker proper-name properties.

\subsection{Vocatives and subjects}

\textcite{Hill2022} examines the syntactic structure of vocative phrases and notes that kinship terms occupy a distinctive position. In vocative function, kinship terms pattern with personal names: \mention{Mom, come here} parallels \mention{Sarah, come here}. Both are direct address forms; both trigger the same intonational contour; both resist modification in the same ways.

What Hill's analysis adds is a structural account: vocative phrases have dedicated syntactic positions, and the forms that occupy those positions share grammatical properties. The vocative slot is a grammaticalization highway~-- a context where high-frequency forms acquire special privileges.
...
\ea\label{ex:pathway}
    \ea Stage 1 (Source): Relational common noun with determiner: \mention{my mother}
    \ex Stage 2 (Bridging): Vocative/address use without determiner: \mention{Mother, can you...}
    \ex Stage 3 (Extension): Referential use in name position: \mention{Mother called}
    \ex Stage 4 (Target): Strong proper name: bare, unique reference, proper-noun distribution
    \z
\z

The bridging context is crucial. Vocative position licenses bare NPs~-- you can say \mention{Waiter!} without a determiner. But vocative frequency alone doesn't explain proper-noun syntax. \mention{Waiter} doesn't license bare singular subjects: \ungram{\mention{Waiter called}}.
...
The process isn't complete, and evidence of gradience supports the usage-based account:
...
\item \mention{Mother} and \mention{Father}, more formal and lower-frequency, are grammatical as bare singulars but marked.
\item \mention{Uncle} and \mention{Auntie} are marginal: \mention{?Uncle said so} requires supportive context.
\item \mention{Cousin}, \mention{Nephew}, \mention{Niece}~-- rarely used in address~-- don't license bare singulars.
\end{itemize}

This hierarchy tracks address frequency. It's not an arbitrary list; it's a prediction of the mechanism. Terms pushed into name position by high-frequency address acquire proper-noun syntax; terms with lower address frequency don't.
...
The English naming taboo is culturally robust but linguistically moderate. Cross-linguistic comparison suggests that the mechanism proposed here should operate with varying force in different languages, depending on the strength of the naming taboo and the structure of the kinship-term system.

\subsection{Stronger taboos: Chinese and Vietnamese}

In Mandarin Chinese, the naming taboo extends far beyond parent-child address. The traditional system of \term{bìhuì} (\enquote{name avoidance}) prohibited speaking or writing the personal names of emperors, ancestors, and senior family members. Children not only can't address parents by first name; historically, they couldn't use any character appearing in a parent's name. The kinship-term system is correspondingly elaborate: distinct terms differentiate maternal from paternal relatives, older from younger siblings, and generation levels with precision exceeding English.

Vietnamese exhibits similar complexity. Kinship terms function as the primary pronoun system, determining not just address but self-reference. A child speaking to a parent refers to themself as \mention{con} (\enquote{child}), not a generic first-person pronoun. The kinship-term inventory is among the most intricate documented, encoding lineage, gender, relative age, and birth order.

These facts generate predictions. If the naming taboo drives proper-noun syntax through high-frequency use in name position, then languages with stronger taboos and more elaborate kinship-term systems should show correspondingly robust proper-name properties for kinship terms. Testing this prediction requires detailed syntactic analysis of kinship terms in these languages~-- work beyond the present paper's scope but predicted by the mechanism.

\subsection{Weaker taboos}

The converse prediction is equally important. In speech communities where the naming taboo is weaker~-- where children sometimes use parents' first names, or where the taboo applies only in formal contexts~-- kinship terms should show less proper-noun-like syntax.

Anecdotal evidence from some Scandinavian families, where first-name use for parents has become less marked, suggests that kinship terms in those households pattern more like ordinary relational nouns. This would be consistent with the mechanism: reduced frequency in name position means reduced grammaticalization pressure.
...
\textbf{Objection:} Many nouns occur in vocative position without acquiring proper-noun syntax. \mention{Waiter!} is a perfectly good vocative, but \ungram{\mention{Waiter called}} is ungrammatical. Why should kinship terms be different?

\textbf{Reply:} The mechanism requires sustained high-frequency use across both vocative and referential positions. \mention{Waiter} occurs vocatively, but it's rarely used as a referential term for a specific individual across repeated interactions. A customer might call out \mention{Waiter!} once at a restaurant; they don't say \mention{Waiter} hundreds of times a day, every day, for years.

Kinship terms differ because the naming taboo forces them into name position \emph{persistently}. The child who can't say \mention{Nick} says \mention{Dad}~-- not just vocatively, but as the default referential form. This sustained frequency produces entrenchment that occasional vocative use doesn't.
...
\textbf{Reply:} Yes, appropriately. \mention{Sir} and \mention{ma'am} are high-frequency address forms with their own grammaticalization histories. They show some proper-name-like properties: \mention{?Sir called} is marginal but better than \ungram{\mention{Waiter called}}. The mechanism predicts exactly this: forms used frequently in address position should show intermediate proper-noun properties.
...
An alternative account would treat proper-name syntax as an inherent lexical property of certain kinship terms. On this view, \mention{Mom} is simply listed in the lexicon as a proper noun, distinct from the common noun \mention{mother}.

This analysis captures the synchronic facts but leaves them unexplained. Why should \mention{Mom} be a proper noun when \mention{mother} isn't? Why the gradience across kinship terms? The lexical-specification account treats as brute fact what the frequency mechanism derives from usage patterns.
...
This paper has connected two facts that existing literature treats as unrelated: the English naming taboo (children can't address parents by first name) and the proper-noun syntax of kinship terms (\\mention{Mom called} is grammatical where \\ungram{\\mention{Neighbor called}} isn't). The connection is causal. The taboo creates the frequency; the frequency creates the syntax.

The mechanism is grammaticalization through usage. When the naming taboo forces children to use kinship terms instead of first names, those terms get used thousands of times in slots prototypically occupied by proper names. Repetition produces entrenchment; entrenchment produces proper-noun distribution. The bare singular licensing of \\mention{Mom} is sedimented behavior~-- the grammatical residue of a social prohibition.
